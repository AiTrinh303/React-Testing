...
describe('ProductDetail', () => {
    ...
    it('should render the products details', async() => {
        render(<ProductDetail productId={product[0].id} />)

        const productName = await screen.findByText(new RegExp(product[0].name))
        expect(productName).toBeInTheDocument()

        const productPrice = await screen.findByText(new RegExp(product[0].price.toString()))
