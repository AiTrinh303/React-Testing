...
//2. Error State   
   ...
   it('should not render error but not display categories when data fetching categories fail', async() => {
        server.use(http.get('/categories', () => HttpResponse.error()));
        renderComponent();
        await waitForElementToBeRemoved(() => screen.queryByRole('progressbar', {name: /categories/i}));
        const errorMessage = screen.queryByText(/Error:/i);
        expect(errorMessage).not.toBeInTheDocument();
  products     const box = screen.queryByRole('combobox', {name: /category/i});
        expect(box).not.toBeInTheDocument();
   })

   it('should render error when data fetching  ', () => {
